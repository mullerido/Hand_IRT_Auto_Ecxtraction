...
function  [fxy_approx, fx_approx, fy_approx] = ALP_2d_Sum_Iter(D1, D2, sigma1, sigma2, f, train_inds)
...
for j=1:size(f,2)
    for k=1:size(f,2)
        f_k =f(:,k);
        f_k_train_inds = train_inds(:,k);
        f_k_test_inds = ~f_k_train_inds;
       
       %new, some not in use
        f_k_train_inds_n = find(train_inds(:,k)==1);
        f_k_test_inds_n = find(train_inds(:,k)==0);
        f_k_mean = mean(f_k(f_k_train_inds_n));
%
        f_k(f_k_test_inds_n)=mean_f_val;
        fy_approx(:,j) = fy_approx(:,j) + K2(j,k)*f_k;
...
%fxy_approx = 0.5*(fx_approx+fy_approx);

%fxy_approx = 0.5*(fx_approx)+0.5*(fy_approx);
fxy_approx = 0.8*(fx_approx)+0.2*(fy_approx);
