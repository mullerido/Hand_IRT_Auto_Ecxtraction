load white_wine_no_label.csv


white = white_wine_no_label(1:2449,1:11);
[Z_wine, mu_wine, sig_wine] = (zscore(white));


train_inds_B = ones(2449,11);
...
[fxy_approx{kk}, fx_approx{kk},fy_approx{kk}] = ALP_2d_Sum_Iter(D1, D2, sigma1, sigma2, f_to_approx, train_inds_B)
if(kk==1)
...
%    f_flat_k_i = reshape(f_multiscale{k_i}, 4898*11,1);
 %   figure; plot(f_train)
 %   hold on;
...
for k_i =1 : kk_ind
 %   figure; surf(f_multiscale{k_i}); title('f-multi'); axis([0 11 0 4898 -1 10]);
