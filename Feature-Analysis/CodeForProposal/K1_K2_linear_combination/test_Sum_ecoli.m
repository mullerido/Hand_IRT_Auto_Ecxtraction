load Ecoli_data_only.csv


ecoli = Ecoli_data_only;%(:,[1:2,5:7]);
[Z_ecoli, mu_ecoli, sig_ecoli] = (zscore(ecoli));


train_inds_B = ones(336,7);
...
for k_i =1 : kk_ind
    ...
    f_flat_k_i = reshape(f_multiscale{k_i}, 336*7,1);
%     figure; plot(f_train)
%     hold on;
...
for k_i =1 : kk_ind
    figure; surf(f_multiscale{k_i}); title('f-multi'); axis([0 7 0 336 -5 20]);
