...
%     imshow(fingerRegion); hold on
    ...
    for tOrentation = allOrientations
        ...
        diagT(:,1) = min(imageSize(1),max(1, diagT(:,1)));
        diagT(:,2) = fingerCenter(2)- cos(tOrentation).*diagxDist;
        diagT(:,2) = min(imageSize(2),max(1, diagT(:,2)));
...
%     plot(diagIn(:,1), diagIn(:,2),'-g','LineWidth',2); 
    ...
    try
        ...
        error('can''t find finger ROI- try to change the radi parameter to include more values');
    end
